...
After lora ran for 5 epochs, the val accuracy was 40%. It actually decreased. And the corresponding test accuracy was 37%. Now, I tried searching for reasons as to why. It could be that given that there are so many more parameters (4M training parameters and 241M total (remaining frozen)), that it can take longer to train. Now this is entirely possible, and so I started training again for another 5 epochs, and at epoch 8, the val accuracy is 23%. Yeah there's no hope in this. But I'll let it continue. And I am honestly unable to think of or search of any reason why this performing so badly.
...
Had the daily standup.
After that, did a final look-through of A2TTS and decided to start working on the code to train. Well, I'll first test out Zero-shot usage. On that note, what kind of training data do we even have? I'll need to ask that from Deepthi mam after zero-shot testing.
Anyways, A2TTS weights are available in AIkosh. The model is 1.6GB. Decently heavy but not much. Since this portal is new, looked through it to gain a better understanding. It seems the simplest way would be to download locally and then upload to kaggle. 
So verified my account and logged in. Then while downloading, apparently I need to send a request to download which I did. Now, I can't be certain when this will be accepted, so I'll look for other options to download from (BharatGen might have as well?) or wait and study more details of the diffusion based generator part. I wonder how it works for speech

The access request might not be accepted actually. I don't know how this works or how active the maintainers might be. I checked the bharatgen page but that also points to aikosh. So while I don't have access, I continued with understanding how diffusion works in speech generation here. I already knew decently well how it worked in image generation. The main points of differences were, first, the "canvas" in images was the RGB image / grid, whereas in speech, it's the mel-spectrogram. It looks like an image with the Y axis being pitch, and X being time. Oh this is the normal audio graph thing. Never knew it was called this. Interesting.
Anyways, it also doesn't start from complete noise, rather centering it based on the text embeddings (which need to be converted to speech). Ah and the duration predictor decides which phoneme takes how much time in the mel-spectrogram.

Started with the Grad-TTS paper, which A2TTS is based on. A2TTS didn't mention the exact details of Grad-TTS in their paper, so this is why I started. The baseline before this utilized Autoregressive generators. Which as we can guess were extremely slow, but they were more accurate in the sounds and intonations done by them. The other options were faster but had weird intonations. So for quite a while there was a trade-off between speed and accuracy. Pretty common, and I guess also seen here.
Grad-TTS was the first time diffusion was used for the feature generator in a TTS model, it was used previously for Vocoders according to the paper (Vocoder is the one that converts the features to actual sound. I'll need to look into its working as well).

The diffusion process in Grad-TTS works very similar to the diffusion in images. So, interestingly, but I guess fair, these guys used a UNet as the backbone. Which is understandable given the paper is from 2021. But diffusion as a field has left UNets behind already. Current papers utilize Diffusion Transformers, atleast in images and language domains. And there is no way there aren't papers using them for TTS as well. So this is a point of further research. I wonder what's the equivalent to the DiT in TTS, and whether the Just Image Transformer from the recent Back to Basics paper can be used. At the very least, x-prediction should definitely be tested. But this all stands on the fact of whether I'll be able to train from scratch, because if we are using existing models, then their backbone's architecture is set in stone. Any changes require complete re-training. Hmm, this is what I'll check after this paper. TTS with Diffusion Transformer or similar backbones.

Interesting, they didn't use the Markov chain formulation from DDPMs, but rather something from Stochastic Differential Equations. I remember reading about this a while back and have the corresponding papers in my to-read list. The equations are slightly difficult so I might need to try them later on if there is not much left to research before model access is granted.
Anyways, the paper again mentions the fact of shaping the initial noise weighted by the input text. They also use an algorithm called Monotonic Alignment Searcher (MAS) for getting the the timing for the phonemes (the duration predictor). While the encoder, MAS and vocoder alone are sufficient for a TTS, the result is extremely monotonic and robotic. The purpose of the diffusion process is to add the minutiae, the minor details of human speech, that show the life in us. The noising and denoising process allow the model to learn and utilize that.

The decision to start the noise from mean of the text embeddings was not just randomly chosen, but rather verified using ablation studies. They experimented the same models but starting from 0 mean noise N(0,I), which required a lot more computational steps to reach even a basic level of quality. 
It also has a "speed dial" of sorts using the Euler scheme on the ODE solver. It normally requires a few 1000 steps, but using the scheme, can be reduced to 10s but at the cost of preciseness and chance of instability, and a slightly worse result. The speed dial is to decide how many steps need to be taken, and how much you want training, inference speed vs accuracy.
The models were evaluated by real human testers who gave a score between 1 to 5 (bigger is better), and the Mean Opinion Score of ground truth data was 4.53. In comparison, the generated audio with a 1000 steps scored 4.44, effectively being indistinguishable from a real human. Even at 100 and 10 steps, the MOS were 4.38 for both. Which is not too bad, considering the much much faster speed of generation. Both 1k steps and 10 steps have their usages. 1k is better when no one is waiting at the end of the line and the quality is paramount, while 10 is necessary for something like live translation devices where you can't delay even a second.
And all that with only around 15M parameters. That is tiny tiny. Very good model, but I need to see more recent modifications to this.